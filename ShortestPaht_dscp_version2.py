...
class shortest_path(app_manager.RyuApp):
		...
		def get_topology_data(self, ev):
				...
				links_list=[(link.src.dpid,link.dst.dpid,{'port':link.src.port_no,'bw':0, 'delay':0, 'lldpdelay':0, 'load':0}) for link in self.switch_mod.links] 
		...
		def switch_features_handler(self, ev):
				...
				bands1 = [ofp_parser.OFPMeterBandDscpRemark(type_=ofp.OFPMBT_DSCP_REMARK, len_=0, rate=500, burst_size=10)]
				bands2 = [ofp_parser.OFPMeterBandDscpRemark(type_=ofp.OFPMBT_DSCP_REMARK, len_=0, rate=100, burst_size=10)]
				bands3 = [ofp_parser.OFPMeterBandDrop(type_=ofp.OFPMBT_DROP, len_=0, rate=750,  burst_size=10)]
				req1 = ofp_parser.OFPMeterMod(datapath=dp, command=ofp.OFPMC_ADD, flags=ofp.OFPMF_KBPS,  meter_id=1, bands=bands1)
				req2 = ofp_parser.OFPMeterMod(datapath=dp, command=ofp.OFPMC_ADD, flags=ofp.OFPMF_KBPS,  meter_id=2, bands=bands2)
				req3 = ofp_parser.OFPMeterMod(datapath=dp, command=ofp.OFPMC_ADD, flags=ofp.OFPMF_KBPS,  meter_id=3, bands=bands3)
				dp.send_msg(req1)
				dp.send_msg(req2)
				dp.send_msg(req3)
		...
		def add_flow(self, dp, cookie=0, match=None, inst=[], table=0, priority=10, idle_timeout=0, hard_timeout=0):
				ofp = dp.ofproto
				ofp_parser = dp.ofproto_parser
		...
		def port_stats_event_handler(self, ev):
				...
				for stat in ev.msg.body:
						...
						if dst_dpid != None:
								...
								self.net[dst_dpid][dpid]['load'] = int((((BW/1000000)*8)/link_capacity)*100)
								#print("link {} -> {} bw: {}".format(dpid,dst_dpid,self.net[dpid][dst_dpid]["bw"]))

								if self.net[dpid][dst_dpid]['load'] <= 60:
										if (dst_dpid,dpid) in self.busy_link:
												self.busy_link.remove((dst_dpid, dpid))
												self.busy_link.remove((dpid, dst_dpid))
								else:
										if (dst_dpid, dpid) not in self.busy_link:
												self.busy_link.append((dst_dpid, dpid))
												self.busy_link.append((dpid, dst_dpid))
		...
		def infos_print(self):
			...
			for l in range(length):
				...
				print("load:{}".format(self.net[src_dpid][dst_dpid]['load']))
				print(self.busy_link)
